...
## v9.5.3
- スマホモード（軽量設定）を追加：初回自動ON（スマホ判定）、手動トグル可能
- スマホ用ズームボタン（＋/−）を追加（指はドラッグ移動）
- 自動再描画を抑制（autoSettleデフォルトOFF）
- 更新確認：右下に v9.5.3 build を表示


## v9.5.4
- スマホでの起動エラー修正：dpr（devicePixelRatio）の初期化順序問題を解消
- スマホモード自動適用を遅延実行（初期描画後に適用）
- 更新確認：右下に v9.5.4 build を表示
...
## v9.5.7
- スマホUIをコンパクト化（デフォルトで折りたたみ表示）
- メニュートグル（≡/×）追加
- スマホ用ズーム（＋/−）を常設し、最後に触った位置を基準にズーム
- 更新確認：右下に v9.5.7 build を表示


## v9.5.9
- スマホUIをさらに小さく：上部パネルはデフォルト折りたたみ
- 左下に浮遊ボタン：≡（メニュー）/ ＋/ −（ズーム）


## v9.6.0
- スマホでパネルが大きく重なる問題を修正：起動直後から確実に折りたたみ
- 折りたたみ時はタイトルと「ひらく」だけ表示
- パネル最大高さを30vhに縮小


## v9.6.1
- 左下の≡ボタンを削除（＋／−のみ）
- 操作パネルは「完全に消す／出す」の2状態（スマホは初期OFF）
- 右下の「？」で操作パネルを表示、×で閉じる
- 操作説明にスマホ向け文言を追加


## v9.6.2
- 上部パネルは完全にON/OFF（中間なし）：「？」で表示、×で閉じる
- 「くわしく」と「つかいかた」を統一：使い方はパネル内に集約（重複削除）／詳細は「設定」へ
- 右下のビルド表示を削除


## v9.6.3
- つかいかたオーバーレイ（helpOverlay）を完全削除（CSS/HTML/JS）


## v9.6.4
- 上部パネルが消えない原因（id不一致／残った折りたたみCSS）を修正
- helpOverlay と verFixed（ビルド表示）を完全削除
- 説明はパネル内の「使い方」に統一、詳細は「設定」へ
