...
## v9.6.8
- 「？」で操作パネルが出ない問題に対して、index.html側に“独立した”トグル処理を追加（SWキャッシュ不整合やapp.js例外でも必ず開閉可能）


## v9.6.9
- 携帯の「＋／−」（左下FAB）を有効化（ズームイン／アウトが確実に動作）
- 使っていない Tip/次回非表示/閉じる のガイダンスUIを削除（説明はパネル内「つかいかた」に統一）


## v9.6.10
- app.js の SyntaxError を修正（エラー表示/例外ハンドラ周りの壊れたブロックを置換）
- 携帯の＋/−ズームボタンを確実にクリック/タップで動作するように再配線
